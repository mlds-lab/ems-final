...
class LinearRegressionOne:
    ...
    def __init__(self, bounds=[-np.inf, np.inf], alpha_lasso=None, alpha_ridge=None, tol=0.00001, random_state=0):
        ...
        self.model_lasso = Lasso(alpha=alpha_lasso, random_state=random_state, max_iter=1000, warm_start=True)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # model_lasso = self.model_lasso(alpha=self.alpha_lasso)

        model_lasso = self.model_lasso
        # model_lasso.set_params(**{"alpha": self.alpha_lasso})
        model_lasso.set_params(alpha=self.alpha_lasso)

        print("coef_init = {}".format(model_lasso.coef_init))
