...
def generate_csv_from_pickle_file_daily(data_directory, pkl_file, indicator_name, results_dir, ground_truth=False):
    """
    ...
    # comment out for no ground truth
    if ground_truth:
...
def generate_csv_from_pickle_file_initial(data_directory, pkl_file, indicator_name, results_dir, ground_truth=False):
    """
...
def main(args):
    ...
    for fname in os.listdir(data_directory):
        ...
        if (experiment_name in fname) and (".d.pkl" in fname):
            ...
            generate_csv_from_pickle_file_daily(data_directory, fname, indicator_name, results_dir, ground_truth)

        elif (experiment_name in fname) and (".pkl" in fname):
            indicator_name = fname[len(experiment_name + "-"):-len(".pkl")]
            generate_csv_from_pickle_file_initial(data_directory, fname, indicator_name, results_dir, ground_truth)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ground-truth", action="store_const", const=True, help="add --ground-truth to output ground truth along with predictions")
