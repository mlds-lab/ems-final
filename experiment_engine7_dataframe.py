...
from user_operations import user_split, get_ids
...
def raw_df_to_train(df, tr_ids, te_ids, params):
    ...
    if (np.any(np.isnan(numeric))):
        ...
        from fancyimpute.iterative_svd import IterativeSVD
        imp = IterativeSVD(verbose=False, rank=rank, init_fill_method="mean", convergence_threshold=1e-5,
                           random_state=42).complete(numeric)
...
class trainTestPerformanceEstimator:
    ...
    def estimate_performance(self, Xtrain, ytrain, Gtrain, Xtest, ytest, Gtest):
        ...
        gs = GridSearchCV(model, this_hyper, scoring=scorer, refit=True, return_train_score=True, cv=cv_splits,
                          verbose=False)
        ...
        print("Pred Extrema: %f %f %f %f" % (
        min(self.yhat_train), max(self.yhat_train), min(self.yhat_test), max(self.yhat_test)))
    ...
    def report(self):
        ...
        try:
            ...
            dffeatures = pd.DataFrame(
                data=list(zip(self.features[indf[:L]], coef[indf[:L]], self.ablation_scores[inds])),
...
def data_frame_to_csv(df, score_column, score_name, prefix="", results_folder="results/"):
    ...
    csv_utility.write_csv_daily(results_folder + "%s" % (prefix), umn_ids, dates, np.array([""] * len(dates)),
                                score_name, vals)
...
def learn_model_get_results(pkl_dir, pkl_file, imputed_file, edd_directory, edd_name, save=False, results_dir="experiment_output"):
    """
    ...
    out1 = pickle.load(open(os.path.join(pkl_dir, imputed_file), "rb"))
    df_imputed = out1["dataframe"]

    # Filter qualtrics and derived streams out
    master_streams1 = df_raw.columns
    cols_to_drop = []
    for s in master_streams1:
        if ("qualtrics" in s) or ("(" in s):
            cols_to_drop.append(s)
    marker_df_raw = df_raw.drop(columns=cols_to_drop)


    df_final = pd.concat([df_raw[cols_to_drop], df_imputed], axis=1, join_axes=[df_raw.index])
    # df_final = df_raw[cols_to_drop].append(df_imputed)
    df_raw = df_final
    ...
    # with open("saved_data_frame.pkl", "wb") as f:
    #     pickle.dump(edd_df_raw, f)
    ...
    tr_ids, te_ids = train_test_split(all_ids, train_size=exp_parameters["train_test_split"], stratify=location,
                                      random_state=11)

    X_tr, y_tr, Q_tr, G_tr, X_te, y_te, Q_te, G_te, MG, features, df_tr, df_te = raw_df_to_train(edd_df_raw.copy(),
                                                                                                 tr_ids, te_ids,
                                                                                                 exp_parameters)
    with open('df_tr.pkl', 'wb') as f:
        pickle.dump(df_tr, f)

    with open('df_te.pkl', 'wb') as f:
        pickle.dump(df_te, f)
    ...
    perf = trainTestPerformanceEstimator(short_name, model, features, exp_parameters["hyper_parameters"],
                                         exp_parameters["cv_folds"], exp_parameters["cv_type"])
...
def parallel_learn_worker(params):
    ...
    imputed_file = full_params['imputed_file']

    return learn_model_get_results(pkl_dir, pkl_file, imputed_file, edd_directory, edd_name, save=save, results_dir=results_dir)
...
def data_filtering(df):
    ...
    tbd = np.invert(df["Missing Indicator"].isna()).as_matrix()
    ...
    na_free = df.dropna(axis=1, thresh=0.3 * df.shape[0], inplace=False)
    tbd2 = df.index.isin(na_free.index)

    df.dropna(axis=1, thresh=0.3 * df.shape[0], inplace=True)
...
def data_imputation(pkl_dir, pkl_file, method, rank):
    ...
    # Filter qualtrics and derived streams out
    master_streams = df.columns
    cols_to_drop = []
    for s in master_streams:
        if ("qualtrics" in s) or ("(" in s):
            cols_to_drop.append(s)
    marker_df_raw = df.drop(columns=cols_to_drop)

    # Fill in the nans on columns that are positive only labels with zeros
    features = list(marker_df_raw.columns)
    zero_fill_columns = []
    for f in features:
        if ".app_usage." in f:
            zero_fill_columns.append(f)
    df_zero_fill = marker_df_raw[zero_fill_columns]
    df_zero_fill = df_zero_fill.fillna(0)
    marker_df_raw[zero_fill_columns] = df_zero_fill
    print(len(zero_fill_columns))

    # Filter users with too few values
    marker_df_raw["Missing Indicator"] = df["org.md2k.data_analysis.feature.phone.driving_total.day"] + \
                                         df["org.md2k.data_analysis.feature.phone.bicycle_total.day"] + \
                                         df["org.md2k.data_analysis.feature.phone.still_total.day"] + \
                                         df["org.md2k.data_analysis.feature.phone.on_foot_total.day"] + \
                                         df["org.md2k.data_analysis.feature.phone.tilting_total.day"] + \
    ...
    marker_df_raw.dropna(axis=0, subset=["Missing Indicator"], inplace=True)
    marker_df_raw = marker_df_raw.drop(columns=["Missing Indicator"])

    print("  ... Contains %d instances with core features available" % (df.shape[0]))

    marker_df_raw.dropna(axis=1, thresh=0.3 * marker_df_raw.shape[0], inplace=True)

    print("  ... Contains %d markers that are >=%.2f pct observed\n" % (marker_df_raw.shape[1], 0.3))

    # python3.6 experiment_engine6.py - -data - file. / master_data_file / master_summary_dataframe - 20180514 - 190154.
    # pkl - -edd - dir. / experiment_definitions_300 / daily / --no - spark - -edd-name conscientiousness.d.json

    # data filtering
    # marker_df_raw, tbd = data_filtering(marker_df_raw)

    # Get original feature
    features = list(marker_df_raw.columns)

    # Run Imputation on data frame
    numeric = marker_df_raw[features].as_matrix()
    ...
    if np.any(np.isnan(numeric)):
        ...
        if method == 'SVD':
            ...
            from fancyimpute.knn import KNN
            imp = KNN(verbose=True, k=rank).complete(numeric)

        else:

            raise ValueError("imputation method invalid!")

        marker_df_raw[features] = pd.DataFrame(data=imp, columns=features, index=marker_df_raw.index)

    # tbd[tbd] = tbd2
    # df.loc[tbd, :] = marker_df_raw
    # return df[features]

    return marker_df_raw
...
def main(args):
    ...
    method = 'SVD'
    rank = 25
    df_imputed = data_imputation(pkl_dir, pkl_file, method, rank)
    imputed_file = 'imp.pkl'
    pickle.dump({"dataframe": df_imputed}, open(os.path.join(pkl_dir, imputed_file), "wb"))
    full_params['imputed_file'] = imputed_file
